...
(function($, undefined) {
    ...
    var ViewBase = ViewElement.extend({
        ...
        setupAnimations: function() {
            var animations = this.animations, i, count = animations.length;
            for (i = 0; i < count; i++) {
                animations[i].setup();
            }
        },
...
(function($, undefined) {
    ...
    }, TOP = "top", TOOLTIP_ANIMATION_DURATION = 150, TOOLTIP_OFFSET = 5, TOOLTIP_SHOW_DELAY = 100, TOOLTIP_HIDE_DELAY = 100, TOUCH_START_NS = "touchstart" + NS, TRIANGLE = "triangle", VALUE = "value", VERTICAL_AREA = "verticalArea", VERTICAL_BULLET = "verticalBullet", VERTICAL_LINE = "verticalLine", WEEKS = "weeks", WHITE = "#fff", X = "x", Y = "y", YEARS = "years", ZERO = "zero", ZOOM_START = "zoomStart", ZOOM = "zoom", ZOOM_END = "zoomEnd", CATEGORICAL_CHARTS = [ BAR, COLUMN, LINE, VERTICAL_LINE, AREA, VERTICAL_AREA, CANDLESTICK, OHLC, BULLET, VERTICAL_BULLET ], XY_CHARTS = [ SCATTER, SCATTER_LINE, BUBBLE ], BASE_UNITS = [ MINUTES, HOURS, DAYS, WEEKS, MONTHS, YEARS ];
    ...
    var Chart = Widget.extend({
        ...
        _modelOptions: function() {
            var chart = this, options = chart.options, element = chart.element, height = math.floor(element.innerHeight()), width = math.floor(element.innerWidth());
            return deepExtend({
        ...
        _mousewheel: function(e) {
            var chart = this, origEvent = e.originalEvent, prevented, delta = 0, totalDelta, state = chart._navState, axes, i, currentAxis, axisName, ranges = {};
        ...
        _mouseleave: function() {
            var chart = this, plotArea = chart._plotArea, crosshairs = plotArea.crosshairs, tooltip = chart._tooltip, i;
            for (i = 0; i < crosshairs.length; i++) {
                crosshairs[i].hide();
            }
        ...
        _destroyView: function() {
            var chart = this, pool = dataviz.IDPool.current, model = chart._model, viewElement = chart._viewElement;
            if (model) {
                model.destroy();
            }
            if (viewElement) {
    ...
    var CategoricalPlotArea = PlotAreaBase.extend({
        ...
        aggregateDateSeries: function(panes) {
            var plotArea = this, series = plotArea.srcSeries || plotArea.series, processedSeries = [], categoryAxis, axisPane, categories, categoryMap, groupIx, categoryIndicies, seriesIx, currentSeries, seriesClone, srcData, data, srcValues, i, categoryIx, pointData, value;
            for (seriesIx = 0; seriesIx < series.length; seriesIx++) {
                currentSeries = series[seriesIx];
    ...
    var BaseTooltip = Class.extend({
        ...
        move: function() {
            var tooltip = this, options = tooltip.options, anchor = tooltip.anchor, element = tooltip.element, chartPadding = tooltip.chartPadding, top, left;
    ...
    var CrosshairTooltip = Class.extend({
        init: function(crosshair, options) {
            ...
            tooltip.options = deepExtend({}, tooltip.options, {
                border: {
            ...
            if (!tooltip.template) {
                tooltip.template = CrosshairTooltip.template = renderTemplate("<div class='" + CSS_PREFIX + "tooltip' " + "style='display:none; position: absolute; font: #= d.font #;" + "border: #= d.border.width #px solid;" + "opacity: #= d.opacity #; filter: alpha(opacity=#= d.opacity * 100 #);'>" + "</div>");
            }
            tooltip.element = $(tooltip.template(tooltip.options)).appendTo(chartElement);
        ...
        move: function() {
            var tooltip = this, anchor = tooltip.anchor, element = tooltip.element, chartPadding = tooltip.chartPadding, top, left;
            top = round(anchor.y + chartPadding.top) + "px";
            left = round(anchor.x + chartPadding.left) + "px";
            element.css({
                top: top,
        ...
        content: function(point) {
            var tooltip = this, options = tooltip.options, axis = tooltip.axis, axisOptions = axis.options, content, value, tooltipTemplate;
        ...
        getAnchor: function(width, height) {
            var tooltip = this, options = tooltip.options, position = options.position, vertical = tooltip.axis.options.vertical, points = tooltip.crosshair.points, fPoint = points[0], sPoint = points[1], halfWidth = width / 2, halfHeight = height / 2, padding = options.padding, x, y;
            if (vertical) {
                if (position === LEFT) {
    ...
    var Selection = Observable.extend({
        ...
        _start: function(e) {
            var that = this, options = that.options, target = $(e.event.target);
        ...
        _move: function(e) {
            ...
            var that = this, state = that._state, options = that.options, fullSpan = options.max - options.min, delta = state.startLocation - e.x.location, range = state.range, span = range.to - range.from, target = state.moveTarget, scale = that.wrapper.width() / fullSpan, offset = math.round(delta / scale);
            if (!target) {
                return;
            }
            e.preventDefault();
            if (target.is(".k-selection")) {
                range.from = math.min(math.max(options.min, options.from - offset), options.max - span);
                range.to = math.min(range.from + span, options.max);
            } else if (target.is(".k-leftHandle")) {
                range.from = math.min(math.max(options.min, options.from - offset), options.max - 1);
                range.to = math.max(range.from + 1, range.to);
            } else if (target.is(".k-rightHandle")) {
                range.to = math.min(math.max(options.min + 1, options.to - offset), options.max);
        ...
        _tap: function(e) {
            var that = this, options = that.options, coords = that.chart._eventCoordinates(e), categoryAxis = that.categoryAxis, categoryIx = categoryAxis.getCategoryIndex(new dataviz.Point2D(coords.x, categoryAxis.box.y1)), span = options.to - options.from, mid = options.from + span / 2, offset = math.round(mid - categoryIx), from, to;
            if (that._state) {
                return;
            }
            e.preventDefault();
            that.chart._unsetActivePoint();
            from = math.min(math.max(options.min, options.from - offset), options.max - span);
            to = math.min(from + span, options.max);
            that.set(from, to);
            that.trigger(SELECT_END);
        },
        move: function(from, to) {
            var that = this, options = that.options, offset = options.offset, padding = options.padding, border = options.selection.border, categoryAxis = that.categoryAxis, leftMaskWidth, rightMaskWidth, box, distance;
        ...
        expandLeft: function(delta) {
            var selection = this, options = selection.options;
            selection.set(math.min(options.from - delta, options.to - 1), options.to);
        },
    ...
    function applyAxisDefaults(options, themeOptions) {
        ...
        each([ CATEGORY, VALUE, X, Y ], function() {
            ...
            axes = $.map(axes, function(axisOptions) {
                ...
                return deepExtend({}, themeAxisDefaults, themeAxisDefaults[axisName], axisDefaults, axisDefaults[axisName], {
                    line: {
                        color: axisColor
                    },
                    labels: {
...
(function($, undefined) {
    // Imports ================================================================
    var kendo = window.kendo, Class = kendo.Class, Observable = kendo.Observable, deepExtend = kendo.deepExtend, math = Math, proxy = $.proxy, dataviz = kendo.dataviz, template = kendo.template, defined = dataviz.defined, Chart = dataviz.ui.Chart, Selection = dataviz.Selection, addDuration = dataviz.addDuration, duration = dataviz.duration, last = dataviz.last, lteDateIndex = dataviz.lteDateIndex, renderTemplate = dataviz.renderTemplate, toDate = dataviz.toDate, toTime = dataviz.toTime;
    // Constants =============================================================
    var AUTO_CATEGORY_WIDTH = 28, CHANGE = "change", CSS_PREFIX = "k-", DRAG = "drag", DRAG_END = "dragEnd", NAVIGATOR_PANE = "_navigator", NAVIGATOR_AXIS = NAVIGATOR_PANE, SELECT_START = "selectStart", SELECT = "select", SELECT_END = "selectEnd", ZOOM_ACCELERATION = 3, ZOOM = "zoom", ZOOM_END = "zoomEnd";
    // Stock chart ===========================================================
    ...
    var Navigator = Observable.extend({
        ...
        redraw: function() {
            this._redrawSelf();
            var navi = this, chart = navi.chart, options = navi.options, axis = navi.mainAxis(), axisClone = clone(axis), groups = axis.options.categories, select = navi.options.select || {}, selection = navi.selection, min = 0, max = groups.length - 1, from = min, to = max;
            if (groups.length > 0) {
                if (select.from) {
        ...
        _drag: function(e) {
            ...
            selection.set(lteDateIndex(groups, from), lteDateIndex(groups, to) + 1);
        ...
        readSelection: function() {
            var navi = this, axis = navi.mainAxis(), groups = axis.options.categories, selection = navi.selection, src = selection.options, dst = navi.options.select;
            dst.from = groups[src.from];
            dst.to = groups[src.to];
        },
        indexToDate: function(index) {
            var navi = this, axis = navi.mainAxis(), groups = axis.options.categories;
            return groups[index];
        },
        filterAxes: function() {
        ...
        _zoom: function(e) {
            var navi = this, chart = navi.chart, delta = e.delta, navigatorAxis = navi.mainAxis(), axis = chart._plotArea.categoryAxis, select = navi.options.select, selection = navi.selection;
            ...
            selection.set(lteDateIndex(navigatorAxis.options.categories, navi.options.select.from), lteDateIndex(navigatorAxis.options.categories, navi.options.select.to));
        ...
        _selectStart: function(e) {
            if (this.chart.trigger(SELECT_START, {
                from: e.from,
                to: e.to
            })) {
                e.preventDefault();
        ...
        _select: function(e) {
            var navi = this;
            navi.showHint(navi.indexToDate(e.from), navi.indexToDate(e.to));
