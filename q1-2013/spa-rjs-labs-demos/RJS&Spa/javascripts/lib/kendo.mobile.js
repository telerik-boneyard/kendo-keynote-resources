    ...
    if (transitions) {
        extend(kendo.fx, {
            ...
            stopQueue: function(element, clearQueue, gotoEnd) {
                var cssValues, taskKeys = element.data("keys"), retainPosition = gotoEnd === false && taskKeys, completeCallback = element.data(COMPLETE_CALLBACK);
                if (retainPosition) {
...
(function($, undefined) {
    ...
    var Popup = Widget.extend({
        ...
        close: function() {
            var that = this, options = that.options, wrap = that.wrapper[0] ? that.wrapper : kendo.wrap(that.element), animation, openEffects, closeEffects;
            if (that.visible()) {
...
(function($, undefined) {
    ...
    var ViewEngine = Observable.extend({
        ...
        showView: function(url, transition) {
            ...
            var that = this, remote = false, showClosure = function(view) {
...
(function($, undefined) {
    ...
    var Pane = Widget.extend({
        ...
        navigate: function(url, transition) {
            var that = this, history = that.history, remoteNavigation;
            if (url === BACK) {
                history.pop();
        ...
        _setupAppLinks: function() {
            this.element.handler(this).on("down", roleSelector(linkRoles), "_mouseup").on("up", roleSelector(buttonRoles), "_mouseup").on("click", roleSelector(linkRoles + " " + buttonRoles), appLinkClick).on("touchstart", roleSelector(buttonRoles), captureGhostClick);
        },
        _mouseup: function(e) {
...
(function($, undefined) {
    var kendo = window.kendo, mobile = kendo.mobile, support = kendo.support, Pane = mobile.ui.Pane, DEFAULT_OS = "ios", OS = support.mobileOS, BERRYPHONEGAP = OS.device == "blackberry" && OS.flatVersion >= 600 && OS.flatVersion < 1e3 && OS.appMode, VERTICAL = "km-vertical", HORIZONTAL = "km-horizontal", MOBILE_PLATFORMS = {
        ...
        usedWithBlock: false
    }), systemMeta = '<meta name="apple-mobile-web-app-capable" content="yes" /> ' + '<meta name="apple-mobile-web-app-status-bar-style" content="black" /> ', viewportMeta = viewportTemplate({
        height: ""
...
(function($, undefined) {
    ...
    var ListView = Widget.extend({
        init: function(element, options) {
            ...
            element.on("down", HIGHLIGHT_SELECTOR, "_highlight").on("move up cancel", HIGHLIGHT_SELECTOR, "_dim").on("up", ITEM_SELECTOR, "_click");
        ...
        refresh: function(e) {
            e = e || {};
            var that = this, element = that.element, options = that.options, dataSource = that.dataSource, view = dataSource.view(), loading = that.loading, appendMethod = "html", contents, groups, data, item;
